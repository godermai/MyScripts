...
Sub GetPolicyManger(outputFileName)
    On Error Resume Next
...
Sub GetPowershellInfo(outputFileName)
    ...
    cmd = "powershell -command Get-NetAdapter -IncludeHidden | Out-File " & outputFileName
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetAdapter -IncludeHidden | fl | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetAdapterAdvancedProperty | fl | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetAdapterBinding -IncludeHidden | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetAdapterBinding -IncludeHidden | fl | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetIpConfiguration -Detailed | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-DnsClientNrptPolicy | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Resolve-DnsName bing.com | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command ping bing.com -4 | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command ping bing.com -6 | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Test-NetConnection bing.com -InformationLevel Detailed | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Test-NetConnection bing.com -InformationLevel Detailed -CommonTCPPort HTTP | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetLbfoTeam | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-VMNetworkAdapter –all | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetRoute | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetIPaddress | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-NetLbfoTeam | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-Service -Name:VMMS | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True

    cmd = "powershell -command Get-VMSwitch | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True
    ...
    cmd = "powershell -command Get-WindowsOptionalFeature -Online | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True 

    cmd = "powershell -command Get-Service | FL | Out-File " & outputFileName & " -Append"
    objShell.Run cmd, 0, True
End Sub
...
Call GetApplicationExportLog(applicationExportFileName)

Call GetSystemExportLog(systemExportFileName)

Call GetHotFixInfo(hotFixInfoFileName)

Call GetCreateBindingMap(createBindingMapFileName)

Call GetServiceLogInfo(serviceInfoFileName)

Call GetPowershellInfo(powershellInfoFileName)

Call GetExistingFiles(evtxInputFilePath, configFolder, dhcpEvtxInputFilePrefix, dhcpEvtxInputFileSuffix)

Call GetExistingFiles(evtxInputFilePath, configFolder, ncsiEvtxInputFilePrefix, ncsiEvtxInputFileSuffix)

Call GetExistingFiles(evtxInputFilePath, configFolder, wcmsvcEvtxInputFilePrefix, wcmsvcEvtxInputFileSuffix)

Call GetExistingFiles(evtxInputFilePath, configFolder, wlanAutoConfigEvtxInputFilePrefix, wlanAutoConfigEvtxInputFileSuffix)

Call GetExistingFiles(serviceEtlFilePath, configFolder, serviceEtlFilePrefix, etlFileSuffix)

Call GetExistingFiles(scmEvmfilePath, configFolder, scmEvmFilePrefix)


Call GetPantherFiles(windowsPantherInputPath, configFolder, windowsPantherOutputFilePrefix, pantherSetupFilePrefix, pantherSetupFileSuffix)
